...
fn native_debug(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
...
// Log simples
fn native_log(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if args.len() != 2 {
...
fn native_typeof(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
    ...
    let type_name = match &args[0] {
        ...
        Value::ThreadFunction { .. } => "thread_function",
...
// Uso de memória
fn native_memory_usage(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if !args.is_empty() {
    ...
    if let Some(process) = system.process(sysinfo::get_current_pid().unwrap()) {
        ...
        Ok(Value::Number(memory_kb as f64))
    } else {
        Ok(Value::Number(0.0))
    }
}
...
// Stack trace
fn native_stack_trace(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if !args.is_empty() {
    ...
    Ok(Value::String(stack_string))
}

// Iniciar cronômetro de performance
fn native_perf_start(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
...
fn native_perf_end(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
...
// Assert genérico
fn native_assert(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
...
// Assert igualdade
fn native_assert_equal(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
...
fn native_assert_not_equal(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if args.len() < 2 || args.len() > 3 {
...
// Assert true
fn native_assert_true(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if args.len() < 1 || args.len() > 2 {
...
// Assert false
fn native_assert_false(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if args.len() < 1 || args.len() > 2 {
...
// Assert tipo
fn native_assert_type(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if args.len() < 2 || args.len() > 3 {
...
// Testar regex
fn native_test_regex(args: &[Value], _manager: &crate::native_modules::NativeModuleManager, _heap: &mut crate::heap::Heap) -> Result<Value, RuntimeError> {
    if args.len() != 2 {
