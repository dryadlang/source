...
use dryad_parser::{Expr, Parser, Stmt};
...
mod increment_decrement_parser_tests {
    ...
    fn test_post_increment_statement() {
        ...
        match &result[0] {
            Stmt::Expression(Expr::PostIncrement(var, _), _) => match var.as_ref() {
    ...
    fn test_post_decrement_statement() {
        ...
        assert_eq!(result.len(), 1);
        match &result[0] {
            Stmt::Expression(Expr::PostDecrement(var, _), _) => match var.as_ref() {
    ...
    fn test_pre_increment_statement() {
        ...
        assert_eq!(result.len(), 1);
        match &result[0] {
            Stmt::Expression(Expr::PreIncrement(var, _), _) => match var.as_ref() {
    ...
    fn test_pre_decrement_statement() {
        ...
        assert_eq!(result.len(), 1);
        match &result[0] {
            Stmt::Expression(Expr::PreDecrement(var, _), _) => match var.as_ref() {
    ...
    fn test_exact_syntax_md_example() {
        ...
        // Primeira declaração
        match &result[0] {
            Stmt::VarDeclaration(name, _, value, _) => {
                assert_eq!(name.identifier_name().unwrap(), "contador");
    ...
    fn test_increment_decrement_in_expressions() {
        ...
        assert_eq!(result.len(), 1);
        match &result[0] {
            Stmt::Assignment(pattern, value, _) => {
                assert_eq!(pattern.identifier_name().unwrap(), "result");
            ...
            Stmt::Assignment(pattern, value, _) => {
                ...
                match value {
                    Expr::Binary(left, operator, right, _) => {
                        ...
                        match right.as_ref() {
                            Expr::Literal(dryad_parser::Literal::Number(n), _) => {
                                assert_eq!(*n, 2.0)
                            }
    ...
    fn test_increment_decrement_complex_expression() {
        ...
        match &result[0] {
            Stmt::Assignment(pattern, value, _) => {
                assert_eq!(pattern.identifier_name().unwrap(), "total");
