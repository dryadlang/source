...
fn test_parse_empty_array() {
    ...
    match &program.statements[0] {
        Stmt::VarDeclaration(name, _, Some(expr), _) => {
            assert_eq!(name.identifier_name().unwrap(), "arr");
            match expr {
        ...
        _ => panic!(
            "Esperado VarDeclaration, encontrado {:?}",
...
fn test_parse_array_with_numbers() {
    ...
    match &program.statements[0] {
        Stmt::VarDeclaration(name, _, Some(expr), _) => {
            assert_eq!(name.identifier_name().unwrap(), "numeros");
            match expr {
                Expr::Array(elements, _) => {
...
fn test_parse_array_access() {
    ...
    match &program.statements[0] {
        Stmt::VarDeclaration(name, _, Some(expr), _) => {
            assert_eq!(name.identifier_name().unwrap(), "valor");
            match expr {
...
fn test_parse_empty_tuple() {
    ...
    match &program.statements[0] {
        Stmt::VarDeclaration(name, _, Some(expr), _) => {
            assert_eq!(name.identifier_name().unwrap(), "vazio");
            match expr {
