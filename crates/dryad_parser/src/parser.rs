...
use crate::ast::{Expr, Literal, Stmt, Program, ClassMember, Visibility, ImportKind, Pattern, MatchArm};
...
impl Parser {
    ...
    fn var_declaration(&mut self) -> Result<Stmt, DryadError> {
        ...
        self.consume_semicolon()?;
        Ok(Stmt::VarDeclaration(name, var_type, initializer, location))
    }
    ...
    fn const_declaration(&mut self) -> Result<Stmt, DryadError> {
        ...
        self.consume_semicolon()?;
        Ok(Stmt::ConstDeclaration(name, const_type, value, location))
    }
        ...
        match &self.peek() {
            ...
            Token::Identifier(name) => {
                ...
                if matches!(self.peek(), Token::Symbol('(')) {
                    ...
                    Ok(Expr::Call(Box::new(Expr::Variable(var_name, location.clone())), args, location))
                } else {
    ...
    fn assignment_statement(&mut self) -> Result<Stmt, DryadError> {
        ...
        match self.advance() {
            Token::Symbol('=') => {
                ...
                let value = self.expression()?;
                Ok(Stmt::Assignment(name, value, location))
            }
            Token::Operator(op) if op == "+=" => {
                ...
                Ok(Stmt::Assignment(name, assignment_value, location))
            }
            Token::Operator(op) if op == "-=" => {
                // x -= value  =>  x = x - value
                let value = self.expression()?;
    ...
    fn while_statement(&mut self) -> Result<Stmt, DryadError> {
        ...
        if !matches!(self.peek(), Token::Symbol('(')) {
            return Err(DryadError::new(
                2050, "Esperado '(' após 'while' - sintaxe: while (condição)"
            ));
        }
        self.advance(); // consume '('
        ...
        if !matches!(self.peek(), Token::Symbol(')')) {
            return Err(DryadError::new(
                2051, "Esperado ')' após condição do while"
            ));
        }
        self.advance(); // consume ')'
        ...
        if !matches!(self.peek(), Token::Symbol('{')) {
            return Err(DryadError::new(
                2052, "Esperado '{' após parênteses do while"
            ));
        }
    ...
    fn do_while_statement(&mut self) -> Result<Stmt, DryadError> {
        ...
        if !matches!(self.peek(), Token::Symbol('(')) {
            return Err(DryadError::new(
                2065, "Esperado '(' após 'while' no do-while - sintaxe: do { ... } while (condição);"
            ));
        }
        self.advance(); // consume '('
        ...
        if !matches!(self.peek(), Token::Symbol(')')) {
            return Err(DryadError::new(
                2066, "Esperado ')' após condição do do-while"
            ));
        }
        self.advance(); // consume ')'
        ...
        if !matches!(self.peek(), Token::Symbol(';')) {
            return Err(DryadError::new(
                2067, "Esperado ';' após parênteses do do-while"
            ));
        }
        self.advance(); // consume ';'
    ...
    fn for_statement(&mut self) -> Result<Stmt, DryadError> {
        ...
        if !matches!(self.peek(), Token::Symbol('(')) {
            return Err(DryadError::new(
                2055, "Esperado '(' após 'for' - sintaxe: for (init; condition; update)"
            ));
        }
        self.advance(); // consume '('
        ...
        let init = if matches!(self.peek(), Token::Symbol(';')) {
            ...
            if let Token::Identifier(var_name) = self.peek() {
                ...
                if !matches!(self.peek(), Token::Symbol('=')) {
                    return Err(DryadError::new(
                        2056, "Esperado '=' na inicialização do for - sintaxe: for (i = 0; ...)"
                    ));
                }
                self.advance(); // consume '='
                
                let expr = self.expression()?;
                Some(Box::new(Stmt::Assignment(var, expr, location.clone())))
            } else {
                return Err(DryadError::new(
                    2057, "Esperado identificador na inicialização do for - sintaxe: for (i = 0; ...)"
                ));
            }
        };
        ...
        if !matches!(self.peek(), Token::Symbol(';')) {
            return Err(DryadError::new(
                2058, "Esperado ';' após inicialização do for - sintaxe: for (i = 0; condition; ...)"
            ));
        }
        self.advance(); // consume ';'
        ...
        if !matches!(self.peek(), Token::Symbol(';')) {
            return Err(DryadError::new(
                2059, "Esperado ';' após condição do for - sintaxe: for (...; i < 10; update)"
            ));
        }
        self.advance(); // consume ';'
        ...
        let update = if matches!(self.peek(), Token::Symbol(')')) {
            ...
            if let Token::Identifier(var_name) = self.peek() {
                ...
                return Err(DryadError::new(
                    2061, "Esperado identificador no update do for - sintaxe: for (...; ...; i++)"
                ));
            }
        };
        ...
        if !matches!(self.peek(), Token::Symbol(')')) {
            return Err(DryadError::new(
                2062, "Esperado ')' após declaração do for - sintaxe: for (init; condition; update)"
            ));
        }
        self.advance(); // consume ')'
        ...
        if !matches!(self.peek(), Token::Symbol('{')) {
            return Err(DryadError::new(
                2063, "Esperado '{' após parênteses do for"
            ));
        }
    ...
    fn foreach_statement(&mut self, var_name: String) -> Result<Stmt, DryadError> {
        ...
        if !matches!(self.peek(), Token::Keyword(ref k) if k == "in") {
            return Err(DryadError::new(
                2068, "Esperado 'in' em foreach loop - sintaxe: for (item in array)"
            ));
        }
        ...
        if !matches!(self.peek(), Token::Symbol(')')) {
            return Err(DryadError::new(
                2069, "Esperado ')' após expressão do foreach - sintaxe: for (item in array)"
            ));
        }
        self.advance(); // consume ')'
        ...
        if !matches!(self.peek(), Token::Symbol('{')) {
            return Err(DryadError::new(
                2070, "Esperado '{' após parênteses do foreach"
            ));
        }
        ...
        Ok(Stmt::ForEach(var_name, iterable, body, location))
    }
    ...
    fn class_member(&mut self) -> Result<ClassMember, DryadError> {
        ...
        match self.peek() {
            ...
            Token::Keyword(k) if k == "let" => {
                ...
                Ok(ClassMember::Property(visibility, is_static, name, prop_type, default_value))
            }
            Token::Identifier(_) => {
    ...
    fn parse_type(&mut self) -> Result<crate::ast::Type, DryadError> {
        ...
        match token {
            Token::Identifier(name) => {
                ...
                match name_str.as_str() {
                    ...
                    _ => {
                        ...
                        if matches!(self.peek(), Token::Symbol('[')) && matches!(self.peek_next(), Token::Symbol(']')) {
                            self.advance(); // [
                            self.advance(); // ]
                            Ok(crate::ast::Type::Array(Box::new(crate::ast::Type::Class(name_str))))
                        } else {
            ...
            Token::Symbol('(') => {
                ...
                if !matches!(self.advance(), Token::Symbol(')')) {
                    return Err(DryadError::new(2100, "Esperado ')' após lista de tipos de tupla"));
                }
                Ok(crate::ast::Type::Tuple(types))
            }
            _ => Err(DryadError::new(2101, &format!("Tipo inválido: {:?}", token))),
        }
    }
                ...
                if !matches!(self.peek(), Token::Symbol(')')) {
                    loop {
                        if let Token::Identifier(param_name) = self.advance() {
                            ...
                            return Err(DryadError::new(2073, "Esperado identificador de parâmetro"));
                        }
    ...
    fn property_assignment_statement(&mut self) -> Result<Stmt, DryadError> {
        ...
        Ok(Stmt::PropertyAssignment(object_expr, property_name, value_expr, location))
    }
    ...
    fn index_assignment_statement(&mut self) -> Result<Stmt, DryadError> {
        ...
        Ok(Stmt::IndexAssignment(array_expr, index_expr, value_expr, location))
    }
    ...
    fn import_statement(&mut self) -> Result<Stmt, DryadError> {
        ...
        match self.peek() {
            ...
            Token::Operator(op) if op == "*" => {
                ...
                self.consume_semicolon()?;
                Ok(Stmt::Import(ImportKind::Namespace(namespace), module_path, location))
            }
            Token::Symbol('{') => {
                ...
                self.consume_semicolon()?;
                Ok(Stmt::Import(ImportKind::Named(imports), module_path, location))
            }
            _ => {
                Err(DryadError::new(
                    4002,
    ...
    fn parse_template_string(&mut self) -> Result<Expr, DryadError> {
        ...
        while !matches!(self.peek(), Token::TemplateEnd) && !self.is_at_end() {
            let part = match self.peek() {
                ...
                Token::InterpolationStart => {
                    ...
                    if !matches!(self.peek(), Token::InterpolationEnd) {
                        return Err(DryadError::new(2027, "Esperado '}' após interpolação em template string"));
                    }
            ...
            expr = match expr {
                Some(prev) => Some(Expr::Binary(Box::new(prev), "+".to_string(), Box::new(part), location.clone())),
                None => Some(part),
            };
        }
    ...
    fn parse_match_expression(&mut self) -> Result<Expr, DryadError> {
        ...
        if !matches!(self.peek(), Token::Symbol('{')) {
            return Err(DryadError::new(2034, "Esperado '{' após expressão do match"));
        }
        self.advance(); // consume '{'
    ...
    fn parse_pattern(&mut self) -> Result<Pattern, DryadError> {
        match self.peek() {
                    ...
                    loop {
                        let key = if let Token::Identifier(k) = self.peek() {
                            ...
                            key_text
                             ...
                             return Err(DryadError::new(2039, "Esperado chave do objeto no padrão"));
                        };
                        
                        if !matches!(self.peek(), Token::Symbol(':')) {
                            return Err(DryadError::new(2040, "Esperado ':' após chave do objeto no padrão"));
                        }
                        self.advance(); // consume ':'
            ...
            _ => Err(DryadError::new(2042, &format!("Padrão inválido: {:?}", self.peek()))),
        }
    }
}
