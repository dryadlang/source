...
use dryad_benchmark::{
    ...
    test_cases::{get_all_test_cases, TestCategory},
...
fn run_all_benchmarks(
    ...
    let profiler = if with_profile { Some(Profiler::new()) } else { None };
...
fn tokenize_code(code: &str) -> Result<Vec<dryad_lexer::token::TokenWithLocation>, Box<dyn std::error::Error>> {
