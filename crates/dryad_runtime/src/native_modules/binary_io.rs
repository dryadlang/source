...
fn extract_bytes_from_value(value: &RuntimeValue) -> Result<Vec<u8>, RuntimeError> {
    match value {
                ...
                match val {
                    RuntimeValue::Number(n) => {
                        if *n < 0.0 || *n > 255.0 || n.fract() != 0.0 {
                            return Err(RuntimeError::ArgumentError(
                                "Valores do array devem ser inteiros entre 0 e 255".to_string()
                            ));
                        }
                        bytes.push(*n as u8);
                    },
                    _ => return Err(RuntimeError::TypeError(
                        "Array deve conter apenas números (bytes)".to_string()
                    ))
                }
            }
            Ok(bytes)
        ...
        RuntimeValue::String(s) => {
            // Permite usar strings como dados binários (UTF-8)
