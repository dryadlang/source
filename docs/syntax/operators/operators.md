...
Desde a versão 0.1.1, o Dryad suporta o operador `::` para acesso a membros de namespace (similar a C++ e Rust).
...
```dryad
// Acesso a função em namespace
ipe::init();

// Acesso a configuração aninhada
config::database::host;

// Uso com objetos (PropertyAccess)
meuModulo::funcao();
meuModulo::propriedade;
```

### Notas

- O operador `::` é tokenizado como dois símbolos `:` consecutivos.
- Internamente, é tratado como `PropertyAccess` na AST.
- Funciona tanto para namespaces quanto para acesso a membros de objetos.
