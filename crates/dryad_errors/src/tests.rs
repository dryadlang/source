...
mod tests {
    ...
    fn test_source_location_with_source_line() {
        let location = SourceLocation::new(None, 1, 1, 0)
            .with_source_line("let x = 5;".to_string());
    ...
    fn test_parser_error_with_expected_tokens() {
        ...
        if let DryadError::Parser { expected: exp, found: f, .. } = &error {
    ...
    fn test_runtime_error_with_stack_trace() {
        ...
        if let DryadError::Runtime { stack_trace: st, .. } = &error {
    ...
    fn test_type_error_creation() {
        ...
        if let DryadError::Type { expected_type, found_type, .. } = &error {
    ...
    fn test_io_error_creation() {
        ...
        if let DryadError::Io { operation, path: p, .. } = &error {
    ...
    fn test_error_with_debug_context() {
        ...
        let error = DryadError::lexer(1002, "Valor inválido", location)
            .with_debug_context(debug_context);
        
        if let DryadError::Lexer { debug_context: Some(ctx), .. } = &error {
    ...
    fn test_stack_trace_display() {
        ...
        let frame1 = StackFrame::new("main".to_string(), SourceLocation::new(None, 1, 1, 0));
        let frame2 = StackFrame::new("calculate".to_string(), SourceLocation::new(None, 10, 5, 150))
    ...
    fn test_auto_context_generation() {
        ...
        if let DryadError::Lexer { debug_context: Some(ctx), .. } = &error {
            ...
            assert!(ctx.suggestions.iter().any(|s| s.contains("Use apenas letras, números, underscore")));
    ...
    fn test_different_error_codes_auto_context() {
        ...
        let error_1003 = DryadError::lexer(1003, "Comentário não fechado", location.clone())
            .with_auto_context();
        
        if let DryadError::Lexer { debug_context: Some(ctx), .. } = &error_1003 {
            let url = ctx.help_url.as_ref().unwrap();
        ...
        if let DryadError::Runtime { debug_context: Some(ctx), .. } = &error_3001 {
            let url = ctx.help_url.as_ref().unwrap();
            assert!(url.contains("e3001-undefined-variable"));
            assert!(ctx.suggestions.iter().any(|s| s.contains("Declare a variável")));
        }
    }
}