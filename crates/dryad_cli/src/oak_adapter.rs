...
impl OakModuleResolver {
    fn resolve_oak_module(&self, module_alias: &str) -> Result<PathBuf, DryadError> {
        ...
        let modules = oaklock.get("modules").ok_or_else(|| {
            DryadError::new(3009, "Seção 'modules' não encontrada no oaklock.json")
        })?;

        let package = modules.get(package_name).ok_or_else(|| {
            DryadError::new(
                3010,
                &format!("Pacote '{}' não encontrado no oaklock.json", package_name),
            )
        })?;

        let paths = package.get("paths").ok_or_else(|| {
            DryadError::new(
                3011,
                &format!(
                    "Seção 'paths' não encontrada para pacote '{}'",
                    package_name
                ),
            )
        })?;
        ...
        let module_path = paths
            ...
            .ok_or_else(|| {
                DryadError::new(
                    3012,
                    &format!(
                        "Módulo '{}' não encontrado no pacote '{}'",
        ...
        if module_path.starts_with("./") || module_path.starts_with("../") {
            ...
            if let Some(current_file) = current_path {
                let base_dir = current_file.parent().ok_or_else(|| {
                    DryadError::new(3004, "Não é possível determinar diretório base")
                })?;
