...
impl IrType {
    ...
    pub fn size(&self) -> u32 {
        match self {
            ...
            IrType::Union { variants } => variants.iter().map(|t| t.size()).max().unwrap_or(0),
    ...
    pub fn align(&self) -> u32 {
        match self {
            ...
            IrType::Struct { fields } => fields.iter().map(|(_, t)| t.align()).max().unwrap_or(1),
            IrType::Union { variants } => variants.iter().map(|t| t.align()).max().unwrap_or(1),
        }
    }
